...
void writeShuntSummary(xlnt::worksheet& ws, const vrfb::shuntcur::pcc::PCCReport& p) {
  ws.column_properties(1).width = 30;
  ws.column_properties(2).width = 15;

  ws.cell(1, 1).value("Arrangement");
  ws.cell(2, 1).value(p.arrName());
  ws.cell(1, 2).value("Number of lines");
  ws.cell(2, 2).value(p.numLines());
  ws.cell(1, 3).value("Number of stacks");
  ws.cell(2, 3).value(p.numStacks());
  ws.cell(1, 4).value("Cells per stack");
  ws.cell(2, 4).value(p.numCells());
  ws.cell(1, 5).value("Resistivity (Ohm m)");
  ws.cell(2, 5).value(p.resistivity());
  ws.cell(1, 6).value("Max charge density (mA cm-2)");
  ws.cell(2, 6).value(p.maxChgDen() / 10);
  ws.cell(1, 7).value("Max charging current (A)");
  ws.cell(2, 7).value(p.maxChgCurr());

  ws.cell(1, 8).value("ASR (cm2 Ohm)");
  ws.cell(2, 8).value(p.asr() * 10000);
  ws.cell(1, 9).value("Cell Area (cm2)");
  ws.cell(2, 9).value(p.cellArea() * 10000);
  ws.cell(1, 10).value("Shunt length (cm)");
  ws.cell(2, 10).value(p.stackShuntLen() * 100);
  ws.cell(1, 11).value("Shunt area (cm2)");
  ws.cell(2, 11).value(p.stackShuntArea() * 10000);
  ws.cell(1, 12).value("Manifold length (cm)");
  ws.cell(2, 12).value(p.stackManiLen() * 100);
  ws.cell(1, 13).value("Manifold area (cm2)");
  ws.cell(2, 13).value(p.stackManiArea() * 10000);

  ws.cell(1, 14).value("Connector (sub) shunt length (cm)");
  ws.cell(2, 14).value(p.connSubShuntLen() * 100);
  ws.cell(1, 15).value("Connector (sub) shunt area (cm2)");
  ws.cell(2, 15).value(p.connSubShuntArea() * 10000);
  ws.cell(1, 16).value("Connector (sub) manifold length (cm)");
  ws.cell(2, 16).value(p.connSubManiLen() * 100);
  ws.cell(1, 17).value("Connector (sub) manifold area (cm2)");
  ws.cell(2, 17).value(p.connSubManiArea() * 10000);
  ws.cell(1, 18).value("Connector (main) shunt length (cm)");
  ws.cell(2, 18).value(p.connMainShuntLen() * 100);
  ws.cell(1, 19).value("Connector (main) shunt area (cm2)");
  ws.cell(2, 19).value(p.connMainShuntArea() * 10000);
  ws.cell(1, 20).value("Connector (main) manifold length (cm)");
  ws.cell(2, 20).value(p.connMainManiLen() * 100);
  ws.cell(1, 21).value("Connector (main) manifold area (cm2)");
  ws.cell(2, 21).value(p.connMainManiArea() * 10000);

  ws.cell(1, 22).value("Charging voltage (V)");
  ws.cell(2, 22).value(p.chargingVolt());
  ws.cell(1, 23).value("Charging current (A)");
  ws.cell(2, 23).value(p.chargingCurr());
  ws.cell(1, 24).value("Charging power (W)");
  ws.cell(2, 24).value(p.chargingPowr());
  ws.cell(1, 25).value("Over voltage loss (W)");
  ws.cell(2, 25).value(p.overVoltPowr());
  ws.cell(1, 26).value("Stored power (W)");
  ws.cell(2, 26).value(p.storedPowr());
  ws.cell(1, 27).value("Power efficiency (%)");
  ws.cell(2, 27).value(p.powrEff() * 100);

  ws.cell(1, 28).value("Norm error");
  ws.cell(2, 28).value(p.err());
}


void writeShuntPerf_Stack(xlnt::worksheet& ws, const vrfb::shuntcur::pcc::PCCReport& p) {
  ws.cell(1, 1).value("Cell No.");
  ws.cell(2, 1).value("Cell Current (A)");
  ws.cell(3, 1).value("Cell Power (W)");
